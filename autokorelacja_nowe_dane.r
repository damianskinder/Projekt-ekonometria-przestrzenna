...
# ── Macierz wag przestrzennych (odległość ekonomiczna) ───────
# Budujemy na podstawie średnich wartości zmiennych ze wszystkich lat

dane_sr <- dane %>%
  group_by(teryt) %>%
  summarise(
    MO      = mean(MO,          na.rm = TRUE),
    W2534   = mean(`WSK25-34`,  na.rm = TRUE),
    WSK_URB = mean(WSK_URB,     na.rm = TRUE),
    NAKL    = mean(`NAKŁ`,      na.rm = TRUE),
    WYNAGR  = mean(WYNAGR,      na.rm = TRUE),
    SM      = mean(SM,          na.rm = TRUE)
  ) %>%
  arrange(teryt)

X <- as.matrix(scale(dane_sr[, -1]))
rownames(X) <- dane_sr$teryt
...
etykiety <- c(
  ...
  "WSK25-34" = "Ludność w wieku 25-34 lat",
...
# ============================================================
#  Test Morana dla wybranego roku (zmień ROK wg potrzeb)
# ============================================================
ROK <- 2015
...
moran.test(dane_rok[["WSK25-34"]], lw, alternative = "two.sided", zero.policy = TRUE)

moran.plot(dane_rok[["WSK25-34"]], lw,
           labels = dane_rok$wojewodztwo,
           pch    = 20, col = "#d6604d",
...
moran.plot(dane_rok$WSK_URB, lw,
           labels = dane_rok$wojewodztwo,
           pch    = 20, col = "#4dac26",
...
# ── 4. Nakłady inwestycyjne w sektorze prywatnym ─────────────
moran.test(dane_rok[["NAKŁ"]], lw, alternative = "two.sided", zero.policy = TRUE)

moran.plot(dane_rok[["NAKŁ"]], lw,
           labels = dane_rok$wojewodztwo,
           pch    = 20, col = "#7b3294",
...
# Wykres zmiany I Morana w czasie
...
# ============================================================
#  Model panelowy FE (efekty stałe)
# ============================================================
pdata <- pdata.frame(dane, index = c("teryt", "rok"))

model_fe <- plm(
  MO ~ WSK_URB + `NAKŁ` + WYNAGR + SM + `WSK25-34`,
  data  = pdata,
  model = "within"
)
