...
ggplot(dane, aes(x = rok, y = zuzycie_energii_GWh, color = wojewodztwo)) +
  ...
  labs(title = "Zużycie energii elektrycznej w gospodarstwach domowych",
       subtitle = "Województwa, 2004–2024", x = "Rok", y = "Zużycie [GWh]", color = "Województwo") +
...
ggplot(dane, aes(x = rok, y = pkb_mln_zl / 1000, color = wojewodztwo)) +
  ...
  scale_color_manual(values = paleta_woj) +
  scale_x_continuous(breaks = seq(2004, 2024, by = 2)) +
  labs(title = "PKB województw", subtitle = "Województwa, 2004–2024",
       x = "Rok", y = "PKB [mld zł]", color = "Województwo") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "right", legend.text = element_text(size = 8),
        panel.grid.minor = element_blank(), plot.title = element_text(face = "bold"))
...
ggplot(dane, aes(x = rok, y = cena_energii_zl_kWh, color = wojewodztwo)) +
  ...
  scale_color_manual(values = paleta_woj) +
  scale_x_continuous(breaks = seq(2004, 2024, by = 2)) +
  labs(title = "Przeciętna cena energii elektrycznej (taryfa G-11)",
       subtitle = "Województwa, 2004–2024", x = "Rok", y = "Cena [zł/kWh]", color = "Województwo") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "right", legend.text = element_text(size = 8),
        panel.grid.minor = element_blank(), plot.title = element_text(face = "bold"))
...
ggplot(dane, aes(x = rok, y = hdd, color = wojewodztwo)) +
  ...
  scale_color_manual(values = paleta_woj) +
  scale_x_continuous(breaks = seq(2004, 2024, by = 2)) +
  labs(title = "Stopniodni grzewcze (HDD)", subtitle = "Województwa, 2004–2024",
       x = "Rok", y = "HDD", color = "Województwo") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "right", legend.text = element_text(size = 8),
        panel.grid.minor = element_blank(), plot.title = element_text(face = "bold"))
...
ggplot(dane, aes(x = rok, y = urbanizacja_pct, color = wojewodztwo)) +
  ...
  labs(title = "Wskaźnik urbanizacji", subtitle = "Województwa, 2004–2024",
       x = "Rok", y = "Ludność miejska [%]", color = "Województwo") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "right", legend.text = element_text(size = 8),
        panel.grid.minor = element_blank(), plot.title = element_text(face = "bold"))
...
anim <- ggplot(dane, aes(x = rok, y = zuzycie_energii_GWh,
  ...
  scale_color_manual(values = paleta_woj) +
  labs(title = "Zużycie energii elektrycznej w gospodarstwach domowych",
       subtitle = "Rok: {frame_along}", x = "Rok", y = "Zużycie [GWh]", color = "Województwo") +
  theme_minimal(base_size = 12) +
...
ggplot(dane_rok, aes(x = zuzycie_energii_GWh)) +
  ...
  labs(title = paste("Rozkład zużycia energii elektrycznej w", rok_wybrany, "roku"),
       subtitle = "Linia przerywana = średnia", x = "Zużycie [GWh]", y = "Liczba województw") +
  theme_minimal(base_size = 12) +
...
ggplot(dane_rok, aes(x = pkb_mln_zl / 1000)) +
  ...
  geom_vline(aes(xintercept = mean(pkb_mln_zl / 1000)),
             color = "#d73027", linewidth = 1, linetype = "dashed") +
  labs(title = paste("Rozkład PKB w", rok_wybrany, "roku"),
       subtitle = "Linia przerywana = średnia", x = "PKB [mld zł]", y = "Liczba województw") +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold"), panel.grid.minor = element_blank())
...
ggplot(dane, aes(x = reorder(wojewodztwo, zuzycie_energii_GWh, median),
  ...
  labs(title = "Zróżnicowanie zużycia energii elektrycznej",
       subtitle = "Województwa, 2004–2024 (posortowane wg mediany)", x = NULL, y = "Zużycie [GWh]") +
...
ggplot(dane, aes(x = reorder(wojewodztwo, pkb_mln_zl, median),
  ...
  geom_boxplot(outlier.shape = 21, outlier.size = 2, alpha = 0.8) +
  scale_fill_manual(values = paleta_woj) +
  labs(title = "Zróżnicowanie PKB województw",
       subtitle = "Województwa, 2004–2024 (posortowane wg mediany)", x = NULL, y = "PKB [mld zł]") +
  coord_flip() + theme_minimal(base_size = 12) +
  theme(legend.position = "none", plot.title = element_text(face = "bold"),
        panel.grid.minor = element_blank())
...
ggplot(dane, aes(x = reorder(wojewodztwo, hdd, median), y = hdd, fill = wojewodztwo)) +
  geom_boxplot(outlier.shape = 21, outlier.size = 2, alpha = 0.8) +
  scale_fill_manual(values = paleta_woj) +
  labs(title = "Zróżnicowanie stopniodni grzewczych (HDD)",
       subtitle = "Województwa, 2004–2024 (posortowane wg mediany)", x = NULL, y = "HDD") +
  coord_flip() + theme_minimal(base_size = 12) +
  theme(legend.position = "none", plot.title = element_text(face = "bold"),
        panel.grid.minor = element_blank())
...
colnames(zmienne_num) <- c("Zużycie energii", "Cena energii", "Ludność",
...
corrplot(cor(zmienne_num, use = "complete.obs"),
         ...
         title = "Macierz korelacji zmiennych panelowych", mar = c(0, 0, 2, 0))
